...
ros::Publisher pub;
ros::Subscriber sub,sub1,sub2,sub3,sub4,sub5;
ros::ServiceServer service,service1,service2,service3;

std_msgs::UInt8MultiArray m;
void apcontrol(const uav_msgs::uav_control &msg){
...
void position(const uav_msgs::uav_position &msg){
    short vx=(short)(msg.vx),vy=(short)(msg.vy);
    short x=short(msg.x),y=(short)(msg.y);
    m.layout.dim[0].label = "speed";
    m.layout.dim[0].size =13;
    m.data.resize(13);
    m.data[0]=0xAA;m.data[1]=0xAF;m.data[2]=0x24;m.data[3]=0x08;
    m.data[4]=BYTE1(vx);m.data[5]=BYTE0(vx);
    m.data[6]=BYTE1(vy);m.data[7]=BYTE0(vy);
    m.data[8]=BYTE1(x);m.data[9]=BYTE0(x);
    m.data[10]=BYTE1(y);m.data[11]=BYTE0(y);
    m.data[12]=0;
...
int main(int argc,char **argv){
	ros::init(argc, argv, "uav_proto_pack");
    ros::NodeHandle n;
