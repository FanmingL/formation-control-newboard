...
{ 
    ...
    for (int i=0;i<len/2;i++){
        sscanf(temp.substr(0,2).c_str(),"%X",(unsigned int *)(temp1+i));
        temp.erase(0,2);
    }
...
bool uart_send(uav_msgs::serial_srv::Request  &req,
               uav_msgs::serial_srv::Response &res)
{
...
bool uart_send_byte(uav_msgs::serial_srv::Request  &req,
                    uav_msgs::serial_srv::Response &res){
    std::string temp=req.input;
    int begin = 0;
...
{
    ros::init(argc, argv, "uav_driver_serial");
